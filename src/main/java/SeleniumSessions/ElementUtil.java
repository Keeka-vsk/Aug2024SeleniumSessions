...
//				}
	...
	/**
	 * An expectation for checking that an element is present on the DOM of a page
	 * this doesn't mean that the element is visible. It is overloaded method of
	 * waitForPresenceOfElement. Here along with Duration Interval time is used.
	 * @param locator
	 * @param timeOut
	 * @param intervaltime
	 * @return
	 ...
	 * An expectation for checking an element is present on the DOM of a page and visible
	 * Visibility means that the element is not only displayed but also has a height and width 
	 * that is greater than 0.It is overloaded method of waitForVisibilityOfElement.
	 * Here along with Duration Interval time is used.
	 * @param locator
	 * @param timeOut
	 * @param intervalTime
	 * @return
	...
	/**
	 * Check for URL
	 * Here you can give the part of url
	 * @param urlFraction
	 * @param timeOut
	 * @return
	 */
	...
	/**
	 * Checks for URL 
	 * Here you have to gave the exact complete url
	 * @param url
	 * @param timeOut
	 * @return
	 */
	...
	public Alert waitForJSAlert(int timeOut) {
		...
		return wait.until(ExpectedConditions.alertIsPresent()); // internally this will switch to the alert
	}
	...
	public void waitForFrameWithFluentWait(String frameIDORName, int timeOut, int intervalTime) {
		...
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver)
			.withTimeout(Duration.ofSeconds(timeOut))
			.pollingEvery(Duration.ofSeconds(intervalTime))
			.withMessage("Time nout done... Frame is  Not found...")
	...
	public Alert waitForJSAlertWithFluentWait(int timeOut, int intervalTime) {
		...
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver)
			.withTimeout(Duration.ofSeconds(timeOut))
			.pollingEvery(Duration.ofSeconds(intervalTime))
			.withMessage("Time nout done... alert is  Not appeared...")
			.ignoring(NoSuchFrameException.class);
	...
	public WebElement retryingElement(By locator, int timeOut) {
		...
		if (element == null) {
			System.err.println("Element is not found... tried for "+ timeOut + "times" + 
